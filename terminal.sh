...
vim ~/.bashrc # изменить алиасы
alias l="ls -ahl" && source ~/.bashrc # установить новый алиас и применить 
# все изменения в текущую сессию терминала (либо можно просто ребутнуть его)



### ГОРЯЧИЕ КЛАВИШИ ###

Tab [двойной] - автокомплит/[подсказки] в git bash
