...
Редактирование:
  {verb}/text_to_search - {verb} до (не вкл.) первого вхождения text_to_search
ПОИСК И ЗАМЕНА
...
Замена:
  ...
  J - "smart join" с разделяющим пробелом
  gJ - "smart join" без разделителя
  gq{motion} - форматирование по максимальной ширине
  Vgq - ... выделенного текста
ПОИСК И ЗАМЕНА
...
Инк(Дек)ремент:
  ...
  Нумерация по порядку:
    1) Заполняем столбец 0-ми;
    2) Выделяем их через <C-q>;
    3) g<Ctrl-a> - все 0 заменяются на порядковые номера от 1 до последнего.
Курсор:
...
<< 'ТЕГИ'
<C-]> - перейти к тегу под курсором
g<C-]> - перейти тегу, использующемуся в нескольких файлах и местах текущего
файла в виде списка, под курсором
<C-t> - перейти назад по стеку тегов
ТЕГИ

<< 'РЕГУЛЯРКИ'
/\v - режим "very magic", делающий регулярки в Vim наиболее похожими на обычные
Регулярки:
  "<whole word starts" вместо "\bwhole word starts"
  "whole word ends>" вместо "whole word ends\b"
  "...\C" - регистронезависимость
  "%s/\v^.{0}$\n//" - удалить все пустые строки
РЕГУЛЯРКИ

<< 'ОПЦИИ_КОМАНДЫ_NVIM'
-d - в diff-режиме
-O[N] - открыть N вертикальных сплитов (по умолчанию: одно на файл)
-p[N] - открыть N табов (по умолчанию: один на файл)
-m - режим без записи
-u <config> - другой конфиг
--clean - чистый режим (без конфига, плагинов, shada)
ОПЦИИ_КОМАНДЫ_NVIM

<< 'DIFF'
:windo diffthis/diffoff
:diffput - применить diff из текущего в противоположный
:diffget - ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^, но наоборот
[c, ]c - перемещение между изменениями
DIFF
