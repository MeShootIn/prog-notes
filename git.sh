...
# * длина заголовка коммита дожна быть <= 50 символов;
# * заголовок не оканчивается точкой;
# * TODO
...
# именем 6-значное число. Пример: 100644 (100 - тип объекта: 100 для файла, др.
# числа для символических ссылок, папок, ...; 644 - права: ЛИБО 644 для
# неисполнимого, ЛИБО 755 для исполнимого файла).
...
# (хороший тон)
git add --chmod+=x file.ext # аналогично команде выше, но для
# НЕпроиндексированного
...
# 3) git flow: 1) + 2).
#
# Cherry-pick - применение текущего коммита к другой ветке. Нужно для
# немедленного исправления бага, присутствующего в нескольких ветках.
#
# Подробнее о механике работы:
# https://youtu.be/ydtgQSaUzw0?list=PLDyvV36pndZFHXjXuwA_NywNrVQO0aQqb
