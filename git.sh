...
git commit --amend [--reset-author] # создаёт новый коммит с новым сообщением,
# индексом [, новым автором]
...
git clean [-d] [-x] [-f] # удалить неотслеживаемые файлы  [, директории]
# [, игнорируемые файлы] [, --force (ибо без него не работает)]
