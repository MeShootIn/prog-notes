...
# предотвращения индексации файлов, которые по каким-либо причинам (внесение
# изменений раньше времени, чтобы не перемещать все файлы в какую-нибудь папку
# "todo" или в ".gitignore") не должны попасть в коммит.


# CONVENTIONAL COMMITS #

# Преимущества:
# * интуитивное, краткое и структурированное описание коммита => вход новых
# контрибьюторов, лёгкость сопровождения;
# * автоматическая генерация CHANGELOG'ов;
# * автоматическое определение текущей версии (в формате SemVer) приложения по
# соответствующим типам коммита (<type>);
# * автоматический запуск процессов сборки и публикации;

# Структура (`[]` - в сообщении коммита отсутствуют):
# <type>[(<optional scope>)]: <description>
#
# [<optional body (paragraph 1)>]
# [<optional body (paragraph 2)>]
# ...
# [<optional body (paragraph N)>]
#
# [<optional footer (paragraph 1)>]
# [<optional footer (paragraph 2)>]
# ...
# [<optional footer (paragraph N)>]

# Основные значения `<type>`:
# * `build` - изменения, влияющие на систему сборки или внешние зависимости
# (например: gulp, broccoli, npm, yarn);
# * `ci` - конфиги или скрипты CI (например: Travis, Circle);
# * `docs` - документация;
# * `feat` - новая фича (коррелирует с MINOR в SemVer);
# * `fix` - исправление бага (коррелирует с PATCH в SemVer);
# * `perf` - повышение производительности;
# * `refactor` - рефакторинг структуры кода;
# * `revert` - исправление предшествующего `fix` (<commit_id>), заголовок равен
# заголовку последнего, а тело равно `This reverts commit <commit_id>.`;
# * `style` - рефакторинг, не влияющий на структуру кода (например: лишние
# пробелы, дополнительные запятые);
# * `test` - редактирование тестов;
# * `release` - релиз (типичное сообщение: `release: cut the v15.1.0-next.3
# release`);
# * `chore` - остальные изменения (например: gitignore, gitattributes, prettier,
# editorconfig, changelog, релиз).
#
# NOTE `<type>...!: ...` - важное изменение (`!` в конце) <=> `BREAKING CHANGE`
# в футере (коррелирует с MAJOR в SemVer), но тогда описание важного изменения
# должно быть в body.
#
# scope - область, к которой применяется коммит (например: `(parser)`,
# `(menu)`, `(api)`).
#
# `<description>` - краткое описание по вышеизложенным правилам, но не с
# заглавной буквы.

# body - тело коммита, но уже в свободной форме, с более подробным описанием,
# которое может состоять из нескольких абзацев, разделённых пустой строкой.

# footer - футер коммита, отделяемый от body пустой строкой, который может
# состоять из нескольких абзацев, разделённых пустой строкой.
#
# Структура каждого футера:
# <word token>{`:<Space>`,`<Space>#`}{`<string value>`,`<breaking change
# description>`}
#
# `<word token>` - строка с `-` вместо пробелов, либо `BREAKING CHANGE` или
# `BREAKING-CHANGE`.
# NOTE Лучше не юзать `BREAKING CHANGE`, т.к. это "исключение из правил", а
# такое плохо запоминается.
#
# Примеры: `Reviewed-by: meshootin`, `Resolves #123`.

# NOTE Юниты заголовка футера (`<type>`, ...) и футера (`<word token>`, ...)
...
git add <file>... # добавляем файлы, которые забыли добавить в последний коммит
# Совершаем другие изменения индекса...
git commit --amend [--reset-author] [--no-edit] # создаёт новый коммит (с,
# возможно, новым индексом) [, с новым автором] [, с тем же сообщением]


# ОТКАТ КОММИТА #

# Полезно, если коммит, который хотелось откатить, уже был запушен в удалённый
# репозиторий.

git revert <commit_id> # добавляет новый коммит в данную ветку, состояние
# которого равно состоянию <commit_id>



### ПРАВА НА ФАЙЛЫ ###

# В логе операции перед именем файла 6-значное число (например: git add -A =>
# "create mode 100644 <file>"). Пример: 100644 (100 - тип объекта: 100 для
...
git config --global core.fileMode false # нужно установить, если файловая
# система не поддерживает отдельное право на выполнение

git update-index --chmod=+x -- <file>... # для Windows: принудительно добавляет
# для ПРОИНДЕКСИРОВАННОГО файла право на исполнение в обход файловой системы
# (хороший тон)
...
# NOTE Имя ветки - это алиас для
# NOTE "HEAD" <=> "@" (для оболочек Windows (CMD, PowerShell) нужны кавычки,
# т.к. там это спецсимвол).
# NOTE Можно указывать коммиты через <commit_id>~N, где N - порядковый номер
...
# * {Author, Commit} - имя и email автора изменений (написанного кода) / коммита
# (тот, кто сделал "git commit ...", т.к. у автора изменений может не быть
# прав);
...
git show <commit_id>~2:<file> # выводит (подобно команде `cat`) содержимое файла
# на момент предпредыдущего коммита относительно <commit_id>

git show :<file> # текущая ПРОИНДЕКСИРОВАННАЯ версия файла
...
# Git не трогает неотслеживаемые файлы => для очистки их надо удалять отдельно.
git clean [-d] [-x] {-f, -n} # удалить неотслеживаемые файлы [, директории]
# [, игнорируемые файлы] { --force (ибо без него не работает) / показать список
# затронутых файлов } из рабочей директории
...
# new.ext", т.к. в этот момент Git вычисляет контрольную сумму этих двух файлов,
...
# состояние которой со временем ПОЛНОСТЬЮ удаляется Git.
...
# удалённой ветки и возврат её на момент коммита
...
git push origin :<branch_name> # удаление ветки из удалённого репозитория
...
# Merge-вариант. По умолчанию юзается простой алгоритм "Fast-forward": указатель
# (алиас) текущей ветки записывается в ".git/ORIG_HEAD", а затем он просто
# переносится на <branch_name>.
...
# При изменении имён указателей (веток, HEAD, ...) вся информация записывается в
# ".git/logs/<name>" (например: ".../HEAD").
#
# `git reflog` <=> `git reflog show` (то, что нужно в 90% случаев) <=> `git log
# --oneline -g`
git reflog [HEAD] [--date[=iso]] # вывод reflog'а для HEAD [с датами [в формате
# ISO]]
...
# Прунинг (очистка локального репозитория от веток из удалённого репозитория,
